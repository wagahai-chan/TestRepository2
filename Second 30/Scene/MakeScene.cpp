...
void MakeScene::draw(void)
{
	...
	for (int y = 0; y < 600; y += 50)
	{
		...
		{
			lpSceneMng.AddDrawQue({ IMAGE_ID("縦")[0],x,lpSceneMng.ScreenCenter.y - 75.0,1.0,0,LAYER::CHAR,false });
		}
		lpSceneMng.AddDrawQue({ IMAGE_ID("横")[0],lpSceneMng.ScreenCenter.x,y,1.0,0,LAYER::CHAR,false });
	}
	lpSceneMng.AddDrawQue({ IMAGE_ID("縦")[0],lpSceneMng.ScreenSize.x - 1,lpSceneMng.ScreenCenter.y,1.0,0,LAYER::CHAR,false });
	lpSceneMng.AddDrawQue({ IMAGE_ID("横")[0],lpSceneMng.ScreenCenter.x,lpSceneMng.ScreenSize.y - 1,1.0,0,LAYER::CHAR,false });

	lpSceneMng.AddDrawQue({ IMAGE_ID("ﾌﾞﾛｯｸ")[0],100.0,675.0,1.0,0,LAYER::CHAR,false });
	lpSceneMng.AddDrawQue({ IMAGE_ID("ｷｬﾗ")[0],	150.0,675.0,1.0,0,LAYER::CHAR,false });
	lpSceneMng.AddDrawQue({ IMAGE_ID("鋸")[0],200.0,675.0,1.0,0,LAYER::CHAR,false });
	lpSceneMng.AddDrawQue({ IMAGE_ID("ｹﾞｰﾄ")[0],300.0,675.0,1.0,0,LAYER::CHAR,false });
	lpSceneMng.AddDrawQue({ IMAGE_ID("ｽｲｯﾁ")[0],350.0,675.0,1.0,0,LAYER::CHAR,false });
	lpSceneMng.AddDrawQue({ IMAGE_ID("ﾄﾞﾗﾑ")[0],400.0,675.0,1.0,0,LAYER::CHAR,false });
	lpSceneMng.AddDrawQue({ IMAGE_ID("木")[0],500.0,675.0,0.25,0,LAYER::CHAR,false });
